# IBL Neural Encoding/Decoding from SABLE Image Tokens

This guide walks through the pipeline that connects IBL Neuropixels spiking data to SABLE's
learned per-patch image tokens: fitting encoding models (spikes-from-latents) and decoding
models (latents-from-spikes), compressing image tokens with PCA to make decoding tractable,
and finally reconstructing / rendering decoded tokens back into images and videos.
