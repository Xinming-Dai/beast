...
#SBATCH --mem=48G
#SBATCH -t 0-00:59:00
#SBATCH -J resnet18_cheese3d_extraction
#SBATCH -o /u/xdai3/project3d/SBALE_repo/beast/scripts/resnet_scripts/encoding_decoding/cheese3d/step1_resnet_latent_%j.log
#SBATCH --export=ALL
...
# Extracts ResNet-18 autoencoder per-image latents for the Cheese3D ephys session's eval-layout
# frames (see scripts/sable_scripts/encoding_decoding/cheese3d/step1_extract_eval_frames.sh and
# docs/sable/neural_extraction.md), using the checkpoint trained from
# configs/resnet_ae_18_cheese3d.yaml (TL/TR cameras). Unlike the IBL encoding scripts, EID here
# is the real Cheese3D ephys session id, not a checkpoint label.
EID="${EID:-20250523_B1_ephys-record_awake_000}"
JOB_ID="${JOB_ID:-21707446}"
MODEL_DIR="${MODEL_DIR:-/projects/bfsr/xdai3/project3d/twoview3d_ckpts/resnet_ae_18/cheese3d/$JOB_ID}"
...
echo "[$(date +'%Y-%m-%d %H:%M:%S')] Extracting ResNet-18 per-image latents -> $OUTPUT_DIR"
