...
#SBATCH --mem=30G
#SBATCH -t 0-04:59:00
#SBATCH -J beast_infer_metrics
#SBATCH -o /u/xdai3/project3d/SABLE_repo_3/beast/scripts/beast_scripts/inference/infer_beast_ibl3d_metrics_%j.log
#SBATCH --export=ALL

# PSNR/SSIM evaluation of a beast ViT autoencoder's reconstructions against their inputs, on
# the same DATASET_PATH/SESSION_NAMES as
...
# OUTPUT_DIR/{EID}/psnr_ssim_metrics.npz per EID via combine_view_metrics.py.
#
# When NEURAL_INPUT_DIR is set, EIDs with neurally-estimated img_tokens under it get their
# metrics from decode_beast_tokens.py's K/T/V neural-trial pipeline instead (one call per EID,
# same as step4_decode_tokens.sh), written to OUTPUT_DIR/{EID}/psnr_ssim_metrics.npz; the flat
# beast-predict metrics (and the merge above) are skipped for those EIDs to avoid a
...
MODEL_DIR="${MODEL_DIR:-/projects/bfsr/xdai3/project3d/twoview3d_ckpts/beast_vit/ibl_pretrain_restricted_sessions/21334260}"
...
# Root directory of per-EID neurally-estimated img_tokens, laid out like
# step4_decode_tokens.sh's MODEL_ROOT (i.e. contains
# latents/img_tokens_compressed/{EID}/img_tokens_compressed_estimated/{EID}/{SPLIT} and a sibling
# img_tokens_camera_parameters.npz ids_restore sidecar). Leave unset to skip the neural K/T/V
# pathway entirely and only run the flat beast-predict metrics below.
...
if [ -n "$NEURAL_INPUT_DIR" ]; then
    ...
    for EID in $SESSION_NAMES; do
        ...
        IDS_RESTORE_SIDECAR="$MODEL_ROOT/img_tokens_camera_parameters.npz"
        ...
        if [ ! -d "$ESTIMATED_DIR" ]; then
            echo "WARNING: skipping $EID, no neurally-estimated img_tokens dir: $ESTIMATED_DIR"
            continue
        fi
        ...
        python -m beast.sable_encoding_decoding.img_token.decode_beast_tokens "${NEURAL_ARGS[@]}"
    done
