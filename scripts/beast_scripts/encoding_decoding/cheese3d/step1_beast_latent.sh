...
#SBATCH --mem=48G
#SBATCH -t 0-02:59:00
#SBATCH -J beast_cheese3d_extraction
#SBATCH -o /u/xdai3/project3d/SBALE_repo/beast/scripts/beast_scripts/encoding_decoding/cheese3d/step1_beast_latent_%j.log
#SBATCH --export=ALL
...
# Extracts beast ViT (base) per-image latents + img_tokens for the Cheese3D ephys session's
# eval-layout frames (see scripts/sable_scripts/encoding_decoding/cheese3d/step1_extract_eval_frames.sh
# and docs/sable/neural_extraction.md), using the checkpoint trained from
# configs/vit_for_SABLE_baseline_cheese3d.yaml (TL/TR cameras). Unlike the IBL encoding scripts,
# EID here is the real Cheese3D ephys session id, not a checkpoint label.
EID="${EID:-20250523_B1_ephys-record_awake_000}"
JOB_ID="${JOB_ID:-21732171}"
MODEL_DIR="${MODEL_DIR:-/projects/bfsr/xdai3/project3d/twoview3d_ckpts/beast_vit/cheese3d/$JOB_ID}"
...
echo "[$(date +'%Y-%m-%d %H:%M:%S')] Extracting beast ViT per-image latents + img_tokens -> $RAW_DIR"
...
# pipeline. --return-img-tokens does not change the saved latents (same CLS token either way;
# see beast.models.vits.VisionTransformer.predict_step), so a single `beast predict` call per
# camera produces both outputs and avoids running the forward pass twice.
...
# merged into one token axis per row), keeping memory under budget
