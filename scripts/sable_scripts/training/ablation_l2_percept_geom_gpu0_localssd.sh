...
#
# Usage:
#   bash ablation_l2_percept_geom_gpu0_localssd.sh
...
# Verify the local NVMe dataset is present before launching.
...
launch_spec() {
  ...
  echo "============================================="
  # Propagate the dataset root to run_one_cell.sh so it builds overrides
  # pointing at local NVMe rather than cephfs.
