"""Reduced-rank regression (with gradient descent) model for neural decoding.

Predicts behavioral/latent covariates `y` from neural rates `X` via a low-rank
...
class RRRGD:
    """Reduced-rank regression model trained with gradient descent (decoder variant).

    Predicts behavioral/latent targets `y: (K, T, ncoef)` from neural covariates
    `X: (K, T, N)` via a shared low-rank temporal basis.

    Parameter layout (per session `eid`, stored in `self.model`, an `nn.ParameterDict`):
        `{eid}_U`: `(ncoef, N - 1, ncomp)` — session-specific loadings over neurons.
        `V`: `(ncomp, T)` — temporal factors, shared across all sessions.
        `{eid}_b`: `(ncoef, 1, T)` — session-specific per-target, per-timestep bias.

    The regression coefficient tensor `beta = U @ V` (concatenated with `b` along the
    coefficient axis when `withbias=True`) has shape `(ncoef, N, T)`.
    """
    ...
    def __init__(self, train_data: dict[str, Any], ncomp: int, l2: float = 0.0) -> None:
        ...
        Args:
            train_data: mapping `eid -> {'X': [train_arr, ...], 'y': [train_arr, ...]}`
                where `train_data[eid]['X'][0]` has shape `(K, T, N)` and
                `train_data[eid]['y'][0]` has shape `(K, T, ncoef)`.
    ...
    def state_dict(self) -> dict[str, Any]:
        ...
        checkpoint = {
            ...
            'l2': self.l2,
            'eids': self.eids,
            # 'N' was never set in the source repo's decoder class (only 'ncoef' is set in
            # __init__), so calling state_dict() would raise AttributeError; fixed by using
            # ncoef, the correct equivalent for this decoder variant.
            'N': self.ncoef,
            'T': self.T,
            'n_comp': self.n_comp,
        }
    ...
    def compute_beta_m(
        ...
        Args:
            U: spatial factors, `(ncoef, N - 1, ncomp)`. Tensor unless `tonp=True`.
            V: temporal factors, `(ncomp, T)`. Tensor unless `tonp=True`.
            b: bias term, `(ncoef, 1, T)`. Tensor unless `tonp=True`.
        ...
        Returns:
            `beta` of shape `(ncoef, N, T)`, as a tensor or numpy array per `tonp`.
        """
        if tonp:
            U = np2tensor(U)
            V = np2tensor(V)
        ...
        if withbias:
            if tonp:
                b = np2tensor(b)
            beta = torch.cat((beta, b), 1)  # (ncoef, N, T)
        else:
    ...
    def compute_beta(self, eid: str, withbias: bool = True) -> torch.Tensor:
        ...
        Returns:
            `beta` of shape `(ncoef, N, T)` for session `eid`.
        """
    ...
    def predict(
        ...
        """Predict behavioral/latent targets from neural covariates and coefficients.

        Args:
            beta: regression coefficients, `(ncoef, N, T)`.
            X: neural covariates, `(K, T, N)`.
            tonp: if `True`, treat inputs as numpy arrays and return a numpy array.

        Returns:
            Predicted targets `y_pred`, `(K, T, ncoef)`.
        """
        if tonp:
            X = np2tensor(X)
            beta = np2tensor(beta)
    ...
    def predict_y(
        ...
        """Predict targets for split `k` of session `eid` (normalized space).
    ...
    def predict_y_behavior(
        self, data: dict[str, Any], eid: str, k: int,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Predict targets for split `k`, de-normalized back to behavioral units.
    ...
    def compute_MSE_RRRGD(self, data: dict[str, Any], k: int) -> dict[str, torch.Tensor]:
        """Compute per-session, per-target summed squared error for split `k`.
        ...
        Returns:
            Mapping `eid -> mse` where `mse` has shape `(ncoef,)`.
        """
        mses_all = {}
        for eid in data:
